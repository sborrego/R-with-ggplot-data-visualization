...
# Factor the values in the column "sex" and write over the values currently in the data frame.
...
# Determine the levels of the factored values. The levels are automatically ordered alphabetically.
levels(mydata$sex)
nlevels(mydata$sex)
```
...
# Filters data by weight and selects columns, creates a new object called "data_small"
data_small <- mydata %>% 
  filter(weight < 5) %>% 
  select(species_id, sex, weight)
